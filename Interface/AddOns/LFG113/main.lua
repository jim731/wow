	...
	function CreatePullDown(parent, x_loc, y_loc, x_width, Instance, InstanceSorted, FuncToCall, multipleLevel)
		--local Default = nil
		local info = UIDropDownMenu_CreateInfo();
		local pullDown = CreateFrame("Frame", nil, parent, "UIDropDownMenuTemplate")
		pullDown:SetPoint("TOPLEFT", x_loc, y_loc)
		...
		UIDropDownMenu_Initialize(pullDown, function (self, level)
			...
			if level == 1 then
				...
				for Index = 1, #InstanceSorted, 1 do
					key, value = InstanceSorted [Index], Instance [InstanceSorted [Index]]
					if ((UnitLevel("player") >= value [3] and UnitLevel("player") <= value [4]) or L.Variables.AllDungeonsChecked or L.Variables.CurrentSearch [key] ~= nil) and value[9] == Level1_Key then
						-- check how far along attunement quests you are
						local attuned = ""
						if (value[10]) then
							local aCount = #value[10]
							for akey, avalue in pairs (value[10]) do
								if (C_QuestLog.IsQuestFlaggedCompleted(avalue)) then return
	...
	function FormatWhisperString (str, playerTo)
		if str then
			...
			local Result = str:gsub ("{i}", myInstances):gsub ("{r}", txtRoles):gsub ("{l}", tostring (UnitLevel ("player"))):gsub ("{c}", tostring (L.Locals.CurrentCommLocal ["txtClasses"][classId])):gsub ("{p}", playerTo):gsub ("{cs}", getTalentSpec())
	...
	function UpdateTableInformation (key, allInstancesCount, specificInstance, Tbl, ToolTip, value)
		...
		if Inst and (Inst:len() > 0 or Inst == "Any Instance") and not memberIsInGroup (key) then
			...
			if not L.Variables.TableRowList[key] then
				...
				ShowNewItemAlert ("|cffFFFFFF" .. key .. ": |cffFF0000" .. ToolTip)
			end
			...
			if next (L.Variables.CurrentSearch) then
				...
				if Tbl[1] == "1" and (L.Variables.TabViewing == 1  or L.Variables.TabViewing == 12) then
					for Index = 1, #AllInstances, 1 do
						...
						if (specificInstance[1] == "3") then AgeRange = { L.Locals.CurrentLocal ["PulldownPVP"][specificInstance[2]][3], L.Locals.CurrentLocal ["PulldownPVP"][specificInstance[2]][4] } end
	...
	function SetRoleIconDisplay (isLFG)
		...
		--class = class:lower()
		if isLFG then
			if L.Variables.PlayerClass [classId] ~= nil then
				L.Variables.CanDPS = L.Variables.PlayerClass [classId]["roles"][3]
				...
				L.Variables.CanHeal = L.Variables.PlayerClass [classId]["roles"][2]
				...
				L.Variables.CanTank = L.Variables.PlayerClass [classId]["roles"][1]
